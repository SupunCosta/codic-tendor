...
public static class PropertiesCache
{
    ...
    private static readonly ConcurrentDictionary<RuntimeTypeHandle, IReadOnlyDictionary<string, string>> ColumnNames =
        new();
    ...
    public static List<PropertyInfo> TypePropertiesAll(Type type)
    {
        if (TypeProperties.TryGetValue(type.TypeHandle, out var cachedProps))
        {
            return cachedProps.ToList();
        ...
        var properties = type.GetProperties().Where(ValidatePropertySelect).ToList();
        TypeProperties[type.TypeHandle] = properties;
        ColumnNames[type.TypeHandle] = GetColumnNames(properties);
    ...
    public static IReadOnlyDictionary<string, string> GetColumnNamesCacheSelect(Type type)
    {
        if (ColumnNames.TryGetValue(type.TypeHandle, out var cachedProps))
        {
            return cachedProps;
    ...
    public static bool ValidateProperty(PropertyInfo prop)
    {
        ...
        result = result && (!prop.PropertyType.IsClass || prop.PropertyType == typeof(string) ||
                            prop.PropertyType == typeof(byte[]) || prop.PropertyType == typeof(DateTime));
    ...
    public static bool ValidatePropertySelect(PropertyInfo prop)
    {
        var result = prop.CanWrite;
        result = result && (prop.GetSetMethod(true)?.IsPublic ?? false);
        // result = result && (!prop.PropertyType.IsClass || prop.PropertyType == typeof(string) ||
    ...
    public static List<PropertyInfo> KeyPropertiesCache(Type type)
    {
        ...
        var keyProperties = allProperties
            .Where(p => p.GetCustomAttributes(true).Any(a => a.GetType().Name == "KeyAttribute")).ToList();
    ...
    public static List<PropertyInfo> ComputedPropertiesCache(Type type)
    {
        ...
        var computedProperties = TypePropertiesCache(type)
            .Where(p => p.GetCustomAttributes(true).Any(a => a.GetType().Name == "ComputedAttribute")).ToList();
    ...
    private static IReadOnlyDictionary<string, string> GetColumnNames(IEnumerable<PropertyInfo> props)
    {
        ...
        {
            var columnAttr = prop.GetCustomAttributes(false)
